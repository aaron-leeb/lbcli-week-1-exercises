...
# WRITE YOUR SOLUTION BELOW:
bitcoin-cli -regtest -named createwallet wallet_name="treasurewallet" descriptors=true

# Generate an address for mining in the treasure wallet
# STUDENT TASK: Generate a new address in the treasurewallet
# WRITE YOUR SOLUTION BELOW:
TREASURE_ADDR=$(bitcoin-cli -regtest -rpcwallet=treasurewallet getnewaddress -addresstype p2sh-segwit)
...
# WRITE YOUR SOLUTION BELOW:
INTERNAL_KEY=$(echo "$ADDR_INFO" | jq -r '.desc' | sed 's/^tr(\(.*\))#.*/\1/')
